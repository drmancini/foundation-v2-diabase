    ...
    pool.on('pool.log', (type, text) => {
      if (type !== 'debug') {
        ...
        if (response.length === 2) {
          ...
          expect(response[0][1]).toBe('Successfully submitted a primary block (Diabase:1) to Diabase\'s daemon instance(s) - found by addr1');
  ...
  test('Test pool manager setup [9]', (done) => {
    ...
    pool.on('pool.log', (type, text) => {
      if (type !== 'debug') {
        ...
        expect(text).toBe('Successfully submitted a primary block (Diabase:1) to Diabase\'s daemon instance(s) - found by addr1');
        done();
      }
    });
    mockSetupDaemons(pool, () => {
